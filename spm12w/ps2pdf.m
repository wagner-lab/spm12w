...
function gsData = LocalParseArgs(varargin) 
    ...
    if ~isfield(gsData, 'cmd') 
        ...
        % ===spm12w edit===
        % Fix to find ghostDir and gsData.cmd on newer matlab versions                        % ===spm12w edit===
        ghostDir = fullfile( matlabroot, 'sys', 'extern', computer('arch'), 'ghostscript' );  % ===spm12w edit===
        if ~isdir(ghostDir)                                                                   % ===spm12w edit===
            ghostDir = fullfile( matlabroot, 'sys', 'gs8x' );   %old matlab ghostdir          % ===spm12w edit===                                  
        end                                                                                   % ===spm12w edit===
        % ===spm12w edit===                                                                        
